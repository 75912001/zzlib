...
void md5_finished(md5_context *ctx, uint8_t digest[16])
{
...
std::string el::lib_md5_t::md5_string( const std::string* in )
{
	md5_context ctx;
	unsigned char md5sum[16] = {0};

	md5_starts(&ctx);
	md5_update(&ctx, (uint8_t *)in->c_str(), (uint32_t)in->size());
	md5_finished(&ctx, md5sum);

	std::string out;
	for (uint32_t i = 0; i < 16; i++){
		char t[3] = {0};
		::sprintf(t, "%02x", md5sum[i]);
		out += t;
	}
	return out;
}
